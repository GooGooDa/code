...
public class FileIOApp2 {
	/*ex01 바탕으로 배열로 코드 리팩토링*/
	public static /* 주석삽입 */ void main(String[] args) throws IOException {
		int [] kors = new int [3]; //이름붙힐때 형식명 + 이름 + new 형식명[] 방의 갯수
		int  total = 0;
		double avg;
		...
		while (answer == 1) {
			...
			case 2: {
				{ // 파일 입력블럭
					...
					String[] korSplit = line.split(","); // line을 콤마로 split(쪼갠다) 이후 배열에 담음
					for(int i=0;i<3;i++)
					kors[i] = Integer.parseInt(korSplit[i]);
					scan.close();
					fis.close();
					// ------------------------------------------------------
